...
class Execution(Strategy):
    ...
    def buy(self, buy_signal_data):
        ...
        #  print(long_df.index[i])
        df["buy_price"] = np.where(df["long_signal"], df["Close"], np.nan)
        df["sl_long"]= np.where(df["long_signal"],df["buy_price"] * (1-sl), np.nan)
        df["tp_long"]= np.where(df["long_signal"], df["buy_price"] * (1+tp), np.nan)

        #filtered only long trades
